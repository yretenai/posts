    ...
    for md_file in reversed(sorted(glob('markdown/*.md'))):
        with cope(md_file, 'r', 'utf8') as md:
            ...
            with cope('docs/{folder}{url}.html'.format(folder=folder, url=name), 'w', 'utf8') as html:
                html.write(POST_TEMPLATE.format(title=meta['title'][0], short=meta['short'][0], url=name, time=upd_date[0], isotime=datetime.strptime(upd_date[0], '%Y-%m-%d %I:%M %p').isoformat(), pub_time=date[0], pub_isotime=datetime.strptime(date[0], '%Y-%m-%d %I:%M %p').isoformat(), body=text, rel=rel))
