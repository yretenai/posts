...
**Note:** *I do not usually verify if these fixes are still needed, and even if they are I simply add a note that they are no longer needed unless the solution is not viable anymore due to things disappearing. DO NOT blindly use these fixes and workarounds if you do not already suffer from the problems described*
...
### Bloodborne Invisible Geometry (via shadPS4 with patches)
